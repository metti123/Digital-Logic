...
for minterm in minterms:
    try:
        groups[bin(minterm).count('1')].append(bin(minterm)[2:].zfill(size)) #groups the data according to the number of 1's
    except KeyError:
...
for i in sorted(groups.keys()):
    ...
    for j in groups[i]:
        if(count==0):
            print('-' * 50)
            print("%5d:" % i, end="")  # Prints group number
            print("\t\t    %-20d%s"%(int(j,2),j) ) # Prints minterm and its binary representation, eg int('0011',2) = 3
            count=1
        else:
            print(' '*50)
            print("\t\t\t    %-20d%s"%(int(j,2),j) ) # Prints minterm and its binary representation, eg int('0011',2) = 3
...
while True:
    ...
    l = sorted(list(tmp.keys())) #l: [0, 1, 2, 3, 4]
    ...
    print("\nUnmarked elements(Prime Implicants) of this table:",None if len(local_unmarked)==0 else ', '.join(local_unmarked)) # Printing Prime Implicants of current table
